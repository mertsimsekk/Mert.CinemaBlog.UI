...
app.controller("ArticleEditController", function ($scope, $http) {
    ...
    $http({
        method: "GET",
        headers: GetHeader(),
        url: "https://localhost:7165/api/Article/GetById?id=" + $scope.GetUrlParameter("Id")
    }).then(function (response) {
    ...
    $scope.ArticleEdit = function () {
        $http({
            method: "PUT",
            headers: GetHeader(),
            url: "https://localhost:7165/api/Article/Update?articleId=" + $scope.GetUrlParameter("Id"),
            data: $scope.article
        }).then(function (response) {
